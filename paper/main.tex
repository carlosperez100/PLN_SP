...
Lima, Perú\\
carlosperez100@gmail.com}}

\maketitle

\begin{abstract}
La notificación de eventos adversos en una institución peruana de salud depende de un proceso manual y
...
directiva institucional de 2021, sobre 70,000 epicrisis de MIMIC-IV.
...
contexto: el modelo procesa solo el 9.0\% del documento. Contra el consenso de dos evaluadores
independientes, el detector recupera el 94.5\,\%
de los eventos confirmados con una especificidad de
0.538, de modo que opera como filtro de cribado y no
como árbitro. El 25\% de los casos que la codificación
...
a la revisión sistemática de historias. En dicha institución la notificación es manual,
...
La notificación de eventos adversos en la institución de referencia opera de forma reactiva: depende
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{figuras/fig5_flujo.pdf}
\caption{Flujo completo del canal. Banda 1: construcción del corpus por
supervisión débil sobre MIMIC-IV-Note, con la auditoría de validez que
corrigió siete modos de fallo. Banda 2: modelado en dos etapas con
abstención y evaluación honesta (cascada extremo a extremo y contraste con
juicio experto). Banda 3: transferencia al español con el corpus ERSP de
etiqueta de oro y destino aplicado en la matriz de priorización GEMSES.}
\label{fig:flujo}
\end{figure*}

\subsection{Corpus y diseño}
La Figura~\ref{fig:flujo} resume el flujo completo del canal; las
subsecciones siguientes detallan cada banda.
...
\textbf{Diseño final (emparejamiento por nota)} & \textbf{0.770} & \textbf{0.843} & \textbf{0.455} \\
\bottomrule
\end{tabular}
\end{table}

Con el control aplicado, los rasgos de mayor peso son inequívocamente clínicos
(\emph{fall}, \emph{induced}, \emph{ileus}, \emph{revision}, \emph{in stent}, \emph{pressure ulcer}, \emph{restenosis}, \emph{reaction}) y el artefacto documental no aparece entre ellos. El diseño
...
\label{sec:modelos}
La comparación entre familias de modelos exige controlar dos factores que, de
...
que el transformer accede al 9.0\% del documento ---la
epicrisis mediana tiene 3,148 \emph{tokens}--- mientras que
...
texto completo} (F1-macro 0.391), lo que
...
ajustado alcanza F1-macro 0.354 frente a 0.330 del
...
(0.024 puntos) se estima sobre una única
partición, sin prueba de significación.

La ponderación de clases resulta determinante para el transformer: sin ella su
F1-macro desciende a 0.249, una variación del
42\% atribuible en exclusiva al tratamiento del desbalance.
...
lugar de forzar una equiparación que induzca a error. No obstante, para
situar el orden de magnitud del enfoque, la Tabla~\ref{tab:literatura}
contrasta los hallazgos de los estudios de referencia con los resultados
análogos de este trabajo, con la advertencia declarada.

\begin{table*}[!t]
\caption{Posición del presente trabajo frente a la literatura. Comparación
orientativa: los estándares de referencia (revisión manual, GTT, códigos) y
los corpus difieren entre filas, por lo que las cifras no son directamente
equiparables (véase el texto).}
\label{tab:literatura}
\centering
\footnotesize
\begin{tabular}{p{2.6cm}p{3.4cm}p{5.2cm}p{5.2cm}}
\hline
\textbf{Estudio} & \textbf{Corpus y estándar} & \textbf{Hallazgo reportado}
& \textbf{Resultado análogo de este trabajo} \\
\hline
Murff \emph{et al.}, 2011, \emph{JAMA} \cite{murff} &
2,974 cirugías (Veterans Affairs); referencia: revisión manual &
Sensibilidad del PLN por complicación 59--91\,\% (neumonía 64\,\%, sepsis
89\,\%, insuf. renal 82\,\%); los indicadores por códigos: 5--46\,\% &
Mismo régimen de funcionamiento: sens. 0.762 frente a códigos y
\textbf{0.945 frente a consenso experto} (n=68), con esp. 0.770 y 0.538
respectivamente \\
\hline
Classen \emph{et al.}, 2011, \emph{Health Affairs} \cite{classen} &
795 admisiones, 3 hospitales; GTT + revisión &
Eventos adversos en el 33\,\% de las admisiones; la notificación voluntaria
y los PSI \textbf{pierden $\sim$90\,\%} de los eventos &
El subregistro aquí medido con datos propios: el \textbf{25\,\%} de los
casos negativos por codificación son eventos reales para el experto \\
\hline
Zech \emph{et al.}, 2018, \emph{PLoS Med.} \cite{zech} &
Radiografías de tórax, multi-hospital &
AUC interno 0.931 cae a 0.815 fuera del hospital; la red identifica el
hospital de origen en $>$99.9\,\% de los casos &
Paralelo directo en texto: AUC 0.973 sostenido por la plantilla de época
(rasgo dominante no clínico) $\rightarrow$ \textbf{0.843} con el confusor
controlado \\
\hline
Li \emph{et al.}, 2022 \cite{cliniclong} &
Benchmarks de PLN clínico &
Clinical-Longformer (ventana 4,096) supera consistentemente a ClinicalBERT
(512) &
Coherente con el efecto aquí aislado: truncar el texto al alcance de BERT
cuesta $-$28\,\% de F1-macro con idéntica arquitectura léxica \\
\hline
\end{tabular}
\end{table*}
...
La Figura~\ref{fig:confersp} desglosa el comportamiento del clasificador de
naturaleza celda por celda, con predicción fuera de pliegue sobre los
6{,}276 casos. La diagonal domina en las clases mayores (Cuidado del
paciente 94\,\%, Medicación 89\,\%, Gestión de la organización 87\,\%,
infecciones 86\,\%) y las confusiones no son aleatorias sino
\textbf{semánticas}: \emph{Insumos} se desplaza hacia \emph{Dispositivo}
(16\,\% de su fila), \emph{Procedimiento} hacia \emph{Cuidado del paciente}
(17\,\%) y \emph{Comportamiento} hacia \emph{Gestión de la organización}
(20\,\%) --- precisamente los pares que la propia taxonomía deja
colindantes. El modelo duda donde un codificador humano también dudaría, lo
que orienta la mejora hacia reglas de desambiguación específicas de esas
fronteras y no hacia un cambio de arquitectura.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figuras/fig6_confusion_ersp.pdf}
\caption{Matriz de confusión de la naturaleza del evento (9 clases) sobre el
corpus en español, con predicción fuera de pliegue ($n$ = 6{,}276). Cada
celda muestra el conteo y el porcentaje de la fila; la referencia es la
codificación experta (Anexo 02).}
\label{fig:confersp}
\end{figure}
...
Se dispone de 78 casos anotados por dos
...
es el \textbf{consenso}: los 68 casos en que ambos
...
De los 16 casos que la codificación
administrativa clasifica como negativos ---sin código de evento adverso---, el
consenso experto confirma evento real en 4
(25.0\%, IC 95\,\%
10.2\%--49.5\%).
...
con 68 casos, el intervalo de la especificidad es
...
directiva institucional de calidad de 2020 cita en su artículo 5.13 como herramienta de
...
\textbf{Sobre el problema planteado.} El problema original es el subregistro
de eventos adversos ($\sim$72\,\% en la institución de destino): la
información existe en el texto clínico, pero nadie puede leer el 100\,\% de
las notas. Este trabajo demuestra que \textbf{la capa técnica de ese problema
es resoluble}: el sistema recupera eventos que nadie notificó ---el 25\,\% de
los casos que la codificación declaraba negativos resultaron reales--- y
multiplica por 2.26 la eficiencia del revisor a prevalencia real. Lo que este
trabajo no resuelve todavía es la dimensión institucional: esa exige el
piloto en un servicio de calidad, cuyos parámetros operativos (VPP 0.455 a
prevalencia 20.12\,\%) quedan calculados. El \textbf{aporte} es triple:
\emph{(i) metodológico} --- un protocolo de auditoría de validez para
supervisión débil en texto clínico (siete modos de fallo corregidos), con el
confusor de época como caso ejemplar de aprendizaje por atajo;
\emph{(ii) empírico} --- el aislamiento experimental de la ventana de
contexto como factor dominante sobre la arquitectura; y
\emph{(iii) aplicado} --- el primer acople documentado de un canal de PLN con
la taxonomía normativa peruana (Anexos 02 y 03), validado con etiqueta de oro
en español.

El sistema desarrollado \textbf{detecta el
94.5\,\% de los eventos adversos que el consenso
...
(F1-macro 0.391), ejecutable en una estación
...
directo para la gestión: el \textbf{25\% de los casos
...
clínico. El diseño final reporta 0.843 sobre señal genuina, y los
...
procesa solo el 9.0\% de la epicrisis, lo que señala las
...
recupera el 94.5\,\% de los eventos que el
consenso de dos evaluadores confirma, con una especificidad de
0.538. La conclusión defendible no es que el sistema
...
criterio de derivación. Además, el 25\% de los casos
...
\bibitem{directiva02} Institución peruana de salud (referencia
reservada), \emph{Directiva institucional de eventos adversos}, 2021.
Anexo 02: taxonomía de eventos adversos; Anexo 03: escala de severidad.
\bibitem{directiva7} Institución peruana de salud (referencia
reservada), \emph{Directiva institucional de gestión del reporte de
eventos adversos y seguridad del paciente}, 2020.
\bibitem{cifras} Institución peruana de salud (referencia reservada),
\emph{Informativo estadístico institucional}, edición de diciembre de
2025.
\end{thebibliography}
